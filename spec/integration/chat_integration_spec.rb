...
RSpec.describe 'Chat Integration', :vcr do
  describe 'PLLUM chat API integration', :integration do
    ...
    before do
      ...
      allow(client).to receive(:new_chat).and_return({ chat_id: 'test_chat_id', log_id: 'test_log_id' })
      allow(client).to receive(:continue_chat).and_return({ chat_id: 'test_chat_id', log_id: 'test_log_id_updated' })
      allow(client).to receive(:get).and_return('Test response')
    end
  ...
  describe 'using the Conversation class', :integration do
    ...
    before do
      ...
      allow(client).to receive(:new_chat).and_return({ chat_id: 'test_chat_id', log_id: 'test_log_id' })
      allow(client).to receive(:continue_chat).and_return({ chat_id: 'test_chat_id', log_id: 'test_log_id_updated' })
      allow(client).to receive(:get).and_return('Mock API response')
    end
