...
RSpec.describe PLLUM::Conversation do
  ...
  describe "#send_message" do
    ...
    context "when starting a new chat" do
      before do
        allow(client).to receive_messages(new_chat: chat_info, get: response_text)
      end
      ...
      it "supports streaming mode with a block" do
        ...
        expect(client).to receive(:new_chat) do |*_args, **_kwargs, &block|
    ...
    context "when continuing an existing chat" do
      ...
      before do
        allow(client).to receive_messages(continue_chat: continue_info, get: response_text)
      end
